...
class App:
    def __init__(self):
        ...
        # On définit un objet multiprocess safe pour stocker l'image
        self.sharedFrame = ThreadSafeFrame(len(buffer.tobytes()) * 5)

    def run(self):
        print("Starting threads")
        self.cameraProcess = multiprocessing.Process(target=runCameraServer,
                                                     args=(self.sharedVariables, self.sharedFrame))
        self.cameraProcess.start()
        self.flaskProcess = multiprocessing.Process(target=runFlaskServer,
                                                    args=(self.sharedVariables, self.sharedFrame))
